...
class ThreadPool3 extends Object {
  private ObjectFIFO idleWorkers;

  private ThreadPool3Worker[] workerList;

  public ThreadPool3(int numberOfThreads) {
    // make sure that it's at least one
    numberOfThreads = Math.max(1, numberOfThreads);

    idleWorkers = new ObjectFIFO(numberOfThreads);
    workerList = new ThreadPool3Worker[numberOfThreads];

    for (int i = 0; i < workerList.length; i++) {
      workerList[i] = new ThreadPool3Worker(idleWorkers);
    }
  }

  public void execute(Runnable target) throws InterruptedException {
    // block (forever) until a worker is available
    ThreadPool3Worker worker = (ThreadPool3Worker) idleWorkers.remove();
  ...
  public void stopRequestIdleWorkers() {
    try {
      ...
      for (int i = 0; i < idle.length; i++) {
        ((ThreadPool3Worker) idle[i]).stopRequest();
      }
...
class ThreadPool3Worker extends Object {
  ...
  public ThreadPool3Worker(ObjectFIFO idleWorkers) {
